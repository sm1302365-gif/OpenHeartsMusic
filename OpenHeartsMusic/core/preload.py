...
class PreloadManager:
    ...
    async def start_preload(self, chat_id: int, count: int = 2) -> None:
        ...
        """
        from OpenHeartsMusic import queue, yt
        ...
        for track in upcoming_tracks:
            ...
            # Add callback to clean up task and retrieve exceptions
            def _preload_done(t: asyncio.Task, cid=chat_id) -> None:
                self._cleanup_task(cid, t)
                if t.done():
                    t.exception()

            task.add_done_callback(_preload_done)
    ...
    async def _preload_track(self, chat_id: int, track) -> None:
        ...
        """
        from OpenHeartsMusic import yt
