...
from hydrogram.errors import FloodWait, MessageIdInvalid, MessageDeleteForbidden, ChatWriteForbidden

from OpenHeartsMusic import tune, app, config, db, lang, queue, tg, yt
from OpenHeartsMusic.helpers import buttons, utils
from OpenHeartsMusic.helpers._play import checkUB
import asyncio
import logging
...
async def play_hndlr(
    ...
    if force:
        ...
        if await db.get_call(chat_id) or position > 0:
            ...
            try:
                from OpenHeartsMusic import preload
    ...
    if not file.file_path:
        if not file.is_live and not getattr(file, "video", False):
            # Start ffmpeg from YouTube's direct audio URL instead of waiting
            # for the complete file download to finish.
            file.file_path = await yt.stream_url(file.id)
    ...
    try:
        ...
        if "bot" in error_msg.lower() or "sign in" in error_msg.lower():
            await safe_edit(
                ...
                "Solution:\n"
                "• Update YouTube cookies in `OpenHeartsMusic/cookies/` folder\n"
